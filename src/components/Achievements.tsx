...
function AchCard({ ach }: { ach: Achievement }) {
  ...
  return (
    <div
      ...
      className="tilt-card rounded-2xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 hover:border-accent-200 dark:hover:border-accent-800 transition-all"
    >
      ...
      <h3 className="font-semibold text-gray-900 dark:text-white mb-1 leading-snug">{ach.title}</h3>
      <p className="text-xs font-medium text-accent-600 dark:text-accent-400 mb-1">{ach.issuer}</p>
      <p className="text-xs text-gray-400 dark:text-gray-500 font-mono mb-3">{ach.date}</p>
      {ach.description && (
        <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">{ach.description}</p>
      )}
    </div>
  )
}
